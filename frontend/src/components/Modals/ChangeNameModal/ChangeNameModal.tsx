...
export default function ChangeNameModal() {
    const {register, formState: {errors}, handleSubmit} = useForm<IFormChangeName>();
    ...
    <ModalLayout icon={<UserChangeIcon />} title="Редактирование имени">
        <form className={styles['form']} onSubmit={handleSubmit(onSubmit)}>
            <MainInput placeholder="Имя пользователя" {...register('name', {
                required: 'Введите имя' 
            })} errorMessage={errors.name?.message}/>
            <p className={styles['text']}>Вы можете редактировать имя пользователя.</p>
            <p className={styles['text']}>По этой информации вас могут найти другие пользователи.</p>
            <div className={styles['buttons__wrapper']}>
                <ModalButton icon={<BackIcon />} onClick={back} type="button">Назад</ModalButton>
